...
class RenderSystem(
    ...
    // Mappers for reading components
    ...
    override fun process(entityId: Int) {
        // Draw each entity's sprite at its position
        val spriteComp = spriteMapper[entityId]
        val transformComp = transformMapper[entityId]

        spriteBatch.draw(
            spriteComp.region,
            transformComp.position.x,
            transformComp.position.y,
            spriteComp.width * transformComp.scale.x,
            spriteComp.height * transformComp.scale.y
        )
    }
